...
new_tidylda <- function(phi, theta, gamma, alpha, beta, summary, call, ...) {
  ...
  if (ncol(theta) != nrow(phi)) {
    ...
    stop("theta and phi matrices have incompatible dimensionality. ",  
  ...
  if (nrow(phi) != nrow(gamma) | ncol(phi) != ncol(gamma)) {
    ...
    stop("phi and gamma matrices must have the same dimensionality. ",
  ...
  if (length(alpha) > 1 & length(alpha) != ncol(theta)) {
    ...
    stop("Since alpha is a vector, length(alpha) must equal ncol(theta). ",
  ...
  if (is.matrix(beta)) {
    if (ncol(beta) != ncol(phi) | nrow(beta) != nrow(phi)) {
      ...
      stop("Since beta is a matrix, it must have the same dimensionality as phi. ",
  ...
  if (! is.matrix(beta) & length(beta) > 1 & length(beta) != ncol(phi)) {
    ...
    stop("Since beta is a vector, length(beta) must equal ncol(phi). ",
  ...
  if (! inherits(summary, "tbl") & ! inherits(summary, "data.frame")) {
    ...
    stop("class(summary) should be 'tbl' or 'data.frame'. However I see that ",
  ...
  if (sum(c("topic", "prevalence", "coherence", "top_terms") %in% names(summary)) < 4) {
    ...
    stop("summary must have names c('topic', 'prevalence', 'coherence', 'top_terms'). ",
