...
[![Codecov test coverage](https://codecov.io/gh/TommyJones/tidylda/graph/badge.svg)](https://app.codecov.io/gh/TommyJones/tidylda)
<!-- badges: end -->
...
replaced the collapsed Gibbs sampler in version 0.1.0 and is multithreaded,
while still being reproducible via `set.seed()`.
...
Some basic functionality is below. Here, we will use the `tidytext` package to create a document term matrix, fit a topic model, predict topics of unseen documents, and update the model with those new documents.
