...
//
// TOKENS ARRIVE SORTED BY WORD WITHIN EACH DOCUMENT. Three things fall out:
...
// notes section 7 requires. Since Phase 4 builds straight from the sparse DTM,
// whose columns already hold their row indices ascending, this costs nothing --
// there is no sort anywhere in construction.
...
class Corpus {
public:
  // Built incrementally, one token at a time, by whoever is walking the DTM.
  //
  // Phase 4 (D16) folded initialization into the engine: the caller iterates the
  // sparse DTM directly and samples each token's initial topic as it goes, so
  // nothing is ever materialized as the vector<vector<size_t>> pair that used to
  // be marshalled out to R and straight back in at 16 bytes per token.
  //
  ...
  // a sparse column already stores its row indices ascending, which is also the
  // order the doc pass wants (consecutive tokens tend to share a word) and what
  // lets the log-likelihood scan runs of distinct (d,v) pairs -- the O(nnz*K)
  // rather than O(N*K) form design notes section 7 requires.
  Corpus(std::size_t n_docs,
         std::size_t n_words,
         std::size_t n_tokens,
         std::size_t mh_steps)
      : n_docs_(n_docs),
  ...
  // and at iteration 1 no pass has run to produce one --- so any other value here
